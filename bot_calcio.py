...
# Inserita la virgoletta mancante
API_KEY = "ea1f03fb102749fa9140e20b184f2996" 
BASE_URL = "https://api.football-data.org/v4/"

# --- 2. SETUP UI & CSS ---
st.set_page_config(page_title="AI NEURAL COMMANDER v11.9", layout="wide", initial_sidebar_state="collapsed")
...
# --- 5. MAIN APP ---
st.markdown("<h1 style='text-align: center; color: #3b82f6;'>🧠 NEURAL COMMANDER v11.9</h1>", unsafe_allow_html=True)
...
if selected != "---":
    if st.session_state.last_selected != selected:
        ...
        with loading_placeholder.container():
            ...
            steps = ["📡 Connessione Satellitare...", "🧬 Power Index Scan...", "🧠 Simulazione Monte Carlo...", "✅ Analisi Completata."]
    ...
    st.markdown(f"<h2 style='text-align:center;'>{m_data['homeTeam']['name'].upper()} vs {m_data['awayTeam']['name'].upper()}</h2>", unsafe_allow_html=True)
    ...
    with col_main:
        ...
        st.success(f"💡 **AI ADVISORY**: Puntare su {res_labels[np.argmax(metrics['1X2'])]}")
        st.markdown('</div>', unsafe_allow_html=True)
